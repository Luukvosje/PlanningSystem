...
import { MIXED_END_HOUR, MIXED_HOUR_WIDTH, MIXED_START_HOUR, timeToMixedPx  } from '~/utils/planning/mixedTimelineMath';
...
const dayTabFormatter = computed(() => new Intl.DateTimeFormat(getIntlLocale(locale.value), { weekday: 'short', day: 'numeric' }));

const mixedAvailabilityOverlays = computed(() => {
  if (!props.availabilityPeriods?.length) {
return [];
}
...
<template>
	<div class="flex flex-col gap-3 h-full min-h-0">
		...
		<div class="relative flex-1 min-h-0 overflow-auto rounded-xl border border-default bg-default shadow-sm">
			<PlanningLoading
				v-if="isLoading"
				:label="t('planning.loadingShifts')"
			/>
