...
return undefined;
  ...
  if (isConcept.value) {
return CONCEPT_BLOCK_STYLE.backgroundColor;
}
...
const tooltipText = computed(() => {
  ...
  parts.push(formatTimeRange(props.record.startUtc, props.record.endUtc, intlLocale.value));
  if (props.record.notes) {
parts.push(props.record.notes);
}
  const warning = checkRecord(props.record);
  if (warning.hasConflict && warning.message) {
	...
	>
		...
		>
			<p class="text-xs font-semibold truncate leading-tight shrink-0">
				{{ isOpen ? t('planning.openShift') : record.assignedUserName }}
			</p>
			<p
