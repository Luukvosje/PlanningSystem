...
watch(selectedRecord, (record) => {
  if (!record || isCreateMode.value) return
...
watch(() => store.createDraft, (draft) => {
  if (!draft || !isCreateMode.value) return
  form.title = ''
  form.description = ''
  form.notes = ''
...
const rangeStart = computed({
  ...
  set: (value: Date) => { form.startUtc = value.toISOString() },
})

const rangeEnd = computed({
  get: () => new Date(form.endUtc),
  set: (value: Date) => { form.endUtc = value.toISOString() },
})
...
<template>
  <USlideover
    ...
    :title="isCreateMode ? 'Nieuwe planning' : 'Planning details'"
    :ui="{ width: 'max-w-md' }"
    @update:open="(open) => !open && store.closeSidebar()"
  >
    <template #body>
      <div v-if="showForm" class="space-y-4">
        <div v-if="selectedRecord && !isCreateMode" class="flex items-center gap-2">
          <PlanningCardsStatusBadge :status="selectedRecord.status" />
          <UBadge v-if="selectedRecord.hasOverlap" color="error" variant="subtle" size="sm">
            Overlap
          </UBadge>
        </div>
        ...
        <UAlert v-if="assignmentWarning" color="warning" variant="subtle" :title="assignmentWarning" />

        <UFormField label="Titel" name="title" required :error="fieldError('Title')">
          <UInput v-model="form.title" :disabled="!canManage" class="w-full" />
        </UFormField>

        <UFormField label="Medewerker" name="assignedUserId" required :error="fieldError('AssignedUserId')">
          <USelect
        ...
        <UFormField label="Klant" name="customerId" :error="fieldError('CustomerId')">
          <USelect
        ...
        <UFormField label="Periode" name="startUtc" :error="periodFieldError">
          <DateTimeRangePicker
        ...
        <UFormField v-if="!isCreateMode" label="Status" name="status" :error="fieldError('Status')">
          <USelect
            v-model="form.status"
            :items="statusOptions"
            value-key="value"
        ...
        <UFormField label="Beschrijving" name="description" :error="fieldError('Description')">
          <UTextarea v-model="form.description" :rows="2" :disabled="!canManage" class="w-full" />
        </UFormField>

        <UFormField label="Notities" name="notes" :error="fieldError('Notes')">
          <UTextarea v-model="form.notes" :rows="4" :disabled="!canManage" class="w-full" />
        </UFormField>

        <div v-if="canManage" class="flex flex-wrap gap-2 pt-2">
          <UButton
          ...
          <template v-if="!isCreateMode">
            <UButton variant="outline" icon="i-lucide-copy" label="Dupliceer" @click="() => { onDuplicate() }" />
            <UButton variant="outline" color="error" icon="i-lucide-trash-2" label="Verwijder" @click="() => { onDelete() }" />
          </template>
        </div>
      </div>
    </template>
