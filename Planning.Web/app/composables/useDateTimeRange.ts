...
export function formatDateTimeRangeDuration(start: Date, end: Date): string {
...
export function useDateTimeRange(
  ...
  const durationLabel = computed(() => formatDateTimeRangeDuration(start.value, end.value))

  const validationError = computed(() => {
    if (end.value.getTime() <= start.value.getTime()) {
      return 'Einde moet na de start liggen'
    }
    return null
  })
